...
const AdForm: React.FC = () => {
  ...
  const campaignSettings = useSelector(
    (state: RootState) => state.workspace.defaultWorkspace?.campaignSetting
  );
  ...
  useEffect(() => {
    ...
    if (campaignSettings) {
      if (
        campaignSettings.targetAreas &&
        campaignSettings.targetAreas.length > 0
      ) {
        dispatch(
          setAdFormData({
            ...adFormData,
            targetAreas: campaignSettings.targetAreas,
            gender: campaignSettings.gender,
          })
        );
      }
  ...
  const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      ...
      updatedData.scheduleEndDate = `${startDate
        .getDate()
        .toString()
    ...
    if (adFormData.targetArea.trim() !== "") {
      dispatch(
        setAdFormData({
          ...adFormData,
          targetAreas: [
            ...adFormData.targetAreas,
            adFormData.targetArea.trim(),
          ],
  ...
  const validateForm = (): boolean => {
    ...
    if (!adFormData.websiteURL && adFormData.campaignType === "Traffic") {
      ...
      !urlPattern.test(adFormData.websiteURL) &&
      adFormData.campaignType === "Traffic"
    ) {
      errors.websiteURL =
  ...
  return (
    <div className="container">
      ...
      <div className="text-center text-md-end">
        <button
          className="btn btn-outline-light d-inline-flex mt-4"
          onClick={(e) => {
            handleSubmit(e);
          }}
        >
